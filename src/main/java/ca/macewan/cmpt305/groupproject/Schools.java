...
public class Schools {
    ...
    public void loadFromCSV(String csvFileName) throws IOException {
        ...
        for (String[] row : data) {
            // Ensure every row has exactly 19 columns by filling missing values
            row = Arrays.copyOf(row, 19);
            ...
            //Catchment catchment = parseCatchment(row[13]); // Placeholder

            School school = new School(id, year, name, schoolType, address);
            schools.add(school);
